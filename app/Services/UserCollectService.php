...
class UserCollectService {
    ...
    public function getViewedGoodsList($member_id = 0,$shownum = 0) {
        ...
        if ($browselist){
            ...
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_name, goods_marketprice, goods_image, store_id, gc_id');
...
//                    	"gc_id_3"           => $goods_list[$k]['gc_id_3'],
     ...
     * @return array
     */
    public function add($insert_arr){
     ...
     * @return array
     */
    public function addAll($insert_arr){
     ...
     * @return array
     */
    public function getOne($where, $field = '*', $order = '', $group = '') {
     ...
     * @return array
     */
    public function getList($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '') {
        if (is_array($page)){
            if ($page[1] > 0){
    ...
    public function getGuessLikeGoods($member_id = 0,$shownum = 0){
        $shownum = ($t = intval($shownum))>0?$t:0;
        $browseclass_arr = array();//浏览历史影视分类数组
        //如果会员ID存在，则读取数据库浏览历史，或者memcache
        if ($member_id > 0){
            if (!C('cache_open')){//查询数据库
        ...
        if (!$member_id){
            ...
            if(cookie('viewed_goods')){
                ...
                $cookie_arr = array();
                foreach ((array)$cookie_arr_tmp as $k=>$v){
                    $info = explode("-", $v);//每项cookie的值为影视ID-访问时间
                    if (is_numeric($info[0])){
                        $cookie_arr[$info[0]] = intval($info[1]);
                    }
                }
...
//                    $tmp['gc_id_3']          = $v['gc_id_3'];
     ...
     * @param array $goods_info 如果已经获取了影视信息则可传递至函数，避免重复查询
     * @return array
     */
    public function addViewedGoodsToCache($goods_id,$member_id,$store_id = 0,$goods_info = array()) {
        ...
        if (!$goods_info){
            ...
            if (is_array($goods_id)){
                ...
                $goods_infotmp = $model_goods->getGoodsList(array('goods_id'=>array('in',$goods_id)), 'goods_id,gc_id,store_id');
                if (!$goods_infotmp){
                    return array('state'=>true);
                }
                foreach ($goods_infotmp as $k=>$v){
                    if($store_id <> $goods_infotmp['store_id']){//店铺浏览自己的影视不加入浏览历史
                ...
                $goods_infotmp = $model_goods->getGoodsInfoByID($goods_id, 'goods_id,gc_id,store_id');
                if(!$goods_infotmp || $store_id == $goods_infotmp['store_id']){//店铺浏览自己的影视不加入浏览历史
                    return array('state'=>true);
                }
        ...
        if ($goodsid_arr){
            //如果影视ID已经存在则先删除该ID，然后在数组末尾新增该ID
        ...
        if (cookie('viewed_goods')) {//如果cookie已经存在
            ...
            if (!empty($vg_ca) && is_array($vg_ca)) {
                foreach ($vg_ca as $vk => $vv) {
                    $vv_arr = explode('-',$vv);
                    if (in_array($vv_arr[0], $goods_idarr)) {//如果该影视的浏览记录已经存在，则删除它
                        unset($vg_ca[$vk]);
                    }
                }
            } else {
